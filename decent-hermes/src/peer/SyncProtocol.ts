...
export class SyncProtocol {
  ...
  private async mergeSyncedMessages(channelId: string, incoming: SyncedHistoryMessage[]): Promise<void> {
    ...
    // Track which incoming messages are genuinely new so we can notify the agent.
    const newMessages: SyncedHistoryMessage[] = [];
    ...
    // Emit message-received for each new message so the Hermes agent sees them.
    // Without this, messages that arrive via negentropy sync (e.g. offline
    // messages or post-ratchet-reset recoveries) are silently dropped.
    for (const message of newMessages) {
      this.onEvent({ type: 'message-received', channelId, message: message as any });
    }
  }
